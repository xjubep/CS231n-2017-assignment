...
def my_dot(nparr_a, nparr_b):
    ret_li = [[0 for j in range(len(nparr_b[0]))] for i in range(len(nparr_a))]
    for i in range(len(nparr_a)):
        for j in range(len(nparr_b[0])):
            for k in range(len(nparr_a[0])):
                ret_li[i][j] += nparr_a[i][k] * nparr_b[k][j]
    return np.array(ret_li)

def my_sum_axis0(nparr):
    ret = []
    for i in nparr.T:
        ret.append(sum(i))
    return np.array(ret)
...
def my_reshape(nparr, shape):
    if nparr.ndim == 3 and shape[1] == -1:
        ret_s0 = []
        for i in range(shape[0]):
            ret_s1 = []
            for j in range(len(nparr[i])):
                for k in range(len(nparr[i][j])):
                    ret_s1.append(nparr[i][j][k])
            ret_s0.append(ret_s1)
        return np.array(ret_s0)
    
    elif nparr.ndim == 4 and shape[1] == -1:
        ret_s0 = []
        for i in range(shape[0]):
            ret_s1 = []
            for j in range(len(nparr[i])):
                for k in range(len(nparr[i][j])):
                    for m in range(len(nparr[i][j][k])):
                        ret_s1.append(nparr[i][j][k][m])
            ret_s0.append(ret_s1)
        return np.array(ret_s0)

    elif nparr.ndim == 2 and len(shape) == 3:
        ret_s0 = []
        for i in range(len(nparr)):
            ret_s1, ret_s2 = [], []
            for j in range(len(nparr[i])):
                ret_s2.append(nparr[i][j])
                if j % shape[2] == shape[2] - 1:
                    ret_s1.append(ret_s2)
                    ret_s2 = []
            ret_s0.append(ret_s1)
        return np.array(ret_s0)

    elif nparr.ndim == 2 and len(shape) == 2:
        return nparr    
...
def affine_forward(x, w, b):
    ...
    NN = len(x) # 10
    reshaped_input = my_reshape(x, [NN, -1])
    out = my_dot(reshaped_input, w) + b
...
def affine_backward(dout, cache):
    ...
    # dx = np.dot(dout, w.T)
    # # print(dx.shape, x.shape)    # (10, 6) (10, 2, 3)
    ...
    reshaped_x = my_reshape(x, [NN, -1])    
    
    dx = my_dot(dout, w.T)
    dx = my_reshape(dx, x.shape)

    dw = my_dot(reshaped_x.T, dout)
    db = my_sum_axis0(dout)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # # print(x.shape, sample_mean.shape)   # (200, 3) (3, )
        # # print(x.shape, sample_var.shape)    # (200, 3) (3, )
        ...
        sample_square_mean_li = []
        for i in range(len(x.T)):
            row_sum = 0.0
            row_square_sum = 0.0
            for j in range(len(x.T[0])):
                row_sum += x.T[i][j].astype(np.float64)
                row_square_sum += x.T[i][j] * x.T[i][j]
            sample_mean_li.append(row_sum/len(x.T[0]))
            sample_square_mean_li.append(row_square_sum/len(x.T[0]))
        sample_mean = np.array(sample_mean_li)                  # E(x)
        sample_square_mean = np.array(sample_square_mean_li)    # E(x^2)

        sample_var = sample_square_mean - my_square(sample_mean)
 
        normalized_data = (x - sample_mean) / my_sqrt(sample_var + eps)
        out = gamma * normalized_data + beta
        ...
        cache = {
            ...
            'normalized_data': normalized_data,
            'gamma': gamma,
            'ivar': 1./my_sqrt(sample_var + eps),
            'sqrtvar': my_sqrt(sample_var + eps),
        }
        ...
        out = (gamma / my_sqrt(running_var + eps) * x) + (beta - (gamma * running_mean) / my_sqrt(running_var + eps))
...
def batchnorm_backward(dout, cache):
    ...
    dbeta = my_sum_axis0(dout)
    dgamma = my_sum_axis0(dout * normalized_data)

    dxhat = dout * gamma
    
    dxmu1 = dxhat * ivar
    
    divar = my_sum_axis0(dxhat * x_minus_mean)

    dsqrtvar = divar * (-1 / sqrtvar**2)
    
    dvar = dsqrtvar * 0.5 * (1 / sqrtvar)

    ones_like_dout_li = [[1.0 for j in range(len(dout[0]))] for i in range(len(dout))]
    ones_like_dout = np.array(ones_like_dout_li)
    dsq = (1 / N) * dvar * ones_like_dout

    dxmu2 = dsq * 2 * x_minus_mean
    
    dx1 = dxmu1 + dxmu2
    dmu = -1 * my_sum_axis0(dxmu1 + dxmu2) 
    
    dx2 = (1 / N) * dmu * ones_like_dout
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1 / N) * gamma * 1/sqrtvar * ((N * dout) - my_sum_axis0(dout) - (x_minus_mean) * my_square(ivar) * my_sum_axis0(dout * x_minus_mean))
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        mask = np.array([[random.uniform(0, 1) >= p for j in range(x.shape[1])] for i in range(x.shape[0])])
