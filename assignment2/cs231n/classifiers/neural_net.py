...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    fc2_activation_li = [[0 for j in range(W2.shape[1])] for i in range(relu_1_activation.shape[0])]
    for i in range(relu_1_activation.shape[0]):
      for j in range(W2.shape[1]):
        for k in range(relu_1_activation.shape[1]):
          fc2_activation_li[i][j] += relu_1_activation[i][k] * W2[k][j]  
    
    fc2_activation = np.array(fc2_activation_li)
    fc2_activation += b2
    ...
    log_scores = [0 for i in range(shift_scores.shape[0])]
    for i in range(shift_scores.shape[0]):
      sum_score = 0
      for j in range(shift_scores.shape[1]):
        sum_score += math.exp(shift_scores[i][j])
      log_scores[i] = math.log(sum_score) 

    loss = -correct_class_scores + log_scores
    loss = sum(loss)
    loss /= N
    loss += reg * (sum(sum(W1*W1)) + sum(sum(W2*W2)) + sum(b1*b1) + sum(b2*b2))
    ...
    dW2_li = [[0 for j in range(dSoft.shape[1])] for i in range(relu_1_activation.T.shape[0])]
    for i in range(relu_1_activation.T.shape[0]):
      for j in range(dSoft.shape[1]):
        for k in range(relu_1_activation.T.shape[1]):
          dW2_li[i][j] += relu_1_activation.T[i][k] * dSoft[k][j]      
    dW2 = np.array(dW2_li)
    ...
    dW1_li = [[0 for j in range(dRelu1.shape[1])] for i in range(X.T.shape[0])]
    for i in range(X.T.shape[0]):
      for j in range(dRelu1.shape[1]):
        for k in range(X.T.shape[1]):
          dW1_li[i][j] += X.T[i][k] * dRelu1[k][j]      
    dW1 = np.array(dW1_li)
    ...
    grads_b1_li = [0 for i in range(db1.shape[1])]
    for i in range(db1.shape[0]):
        for j in range(db1.shape[1]):
          grads_b1_li[j] += db1[i][j]
    grads['b1'] = np.array(grads_b1_li)
  ...
  def predict(self, X):
    ...
    y_pred_li = [0 for i in range(self.loss(X).shape[0])]
    for i in range(self.loss(X).shape[0]):
      max = self.loss(X)[i][0]
      max_idx = 0
      for j in range(self.loss(X).shape[1]):
        if self.loss(X)[i][j] > max:
          max = self.loss(X)[i][j]
          max_idx = j
      y_pred_li[i] = max_idx
    y_pred = np.array(y_pred_li)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
