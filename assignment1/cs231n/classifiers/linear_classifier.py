...
class LinearClassifier(object):
  ...
  def train(self, X, y, learning_rate=1e-3, reg=1e-5, num_iters=100,
    ...
    for it in range(num_iters):
      ...
      batch_indices = np.array(random.sample(list(X_indices), batch_size))
  ...
  def predict(self, X):
    ...
    pred_scores_li = [[0 for j in range(self.W.shape[1])] for i in range(X.shape[0])]
    for i in range(X.shape[0]):
      for j in range(self.W.shape[1]):
        for k in range(X.shape[1]):
          pred_scores_li[i][j] += X[i][k] * self.W[k][j]
    pred_scores = np.array(pred_scores_li)

    y_pred_li = [0 for i in range(pred_scores.shape[0])]
    for i in range(pred_scores.shape[0]):
      max = pred_scores[i][0]
      max_idx = 0
      for j in range(pred_scores.shape[1]):
        if pred_scores[i][j] > max:
          max = pred_scores[i][j]
          max_idx = j
      y_pred_li[i] = max_idx
